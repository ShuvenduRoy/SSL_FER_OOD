...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.gpu = gpu
    if not torch.cuda.is_available():
        ngpus_per_node = 1  # to avoid divide by zero error
    ...
    if args.dataset in ["imagenet", "FER13", 'KDEF', 'AffectNet', "AffectNetAllFace", 'RAF']:
        image_loader = ImageDatasetLoader(root_path=args.data_dir, num_labels=args.num_labels,
                                          dataset=args.dataset, num_class=args.num_classes, algo=args.algo)
    ...
    if args.rank == 0 and args.distributed:
        torch.distributed.barrier()
    print('training dataset length ', len(lb_dset))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--algo', type=str, default='fullysupervised')
