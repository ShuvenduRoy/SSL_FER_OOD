...
class WideResNet(nn.Module):
    def __init__(self, first_stride, num_classes, depth=28, widen_factor=2, drop_rate=0.0, is_remix=False, projection_head=None, dim_in=512, feat_dim=256):
    ...
    def forward(self, x, ood_test=False, return_projection=False):
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        out = self.block3(out)
        ...
        if ood_test:
            ...
            if self.is_remix:
                ...
                if return_projection:
                    return output, rot_output, F.normalize(self.head(out), dim=1)
                return output, rot_output
            else:
                if return_projection:
                    return output, F.normalize(self.head(out), dim=1)
                return output
